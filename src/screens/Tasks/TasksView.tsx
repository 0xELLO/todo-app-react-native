...
type Props = StackScreenProps<StackParamList, 'Tasks'>;
const TasksView = ({ route, navigation } : Props) => {
    ...
    return (
        ...
        <TouchableOpacity
            ...
            <View style={mainStyles.block}>
                ...
                <Pressable onPress={() => {setOpenEdit(false); item.taskName = name; updateTask(item)}}><Text style={mainStyles.titleBlock}>Save</Text></Pressable>
                </>
                : 
                <>
                <Text style={[styles.btn, completed ? {textDecorationLine: 'line-through'} : {}]} ><Text style={mainStyles.titleBlock} >{name}</Text></Text>
                <CheckBox disabled={false} value={completed} onValueChange={(val) => {setCompleted(val); item.isCompleted = val; updateTask(item)} } />
                <Pressable style={{marginRight: 10}} onPress={() => {setOpenEdit(!openEdit)}}><Text style={mainStyles.titleBlock}>Edit</Text></Pressable>
                <Pressable style={{marginRight: 10}} onPress={() => {item.isArchived = true; updateTask(item);} }><Text style={mainStyles.titleBlock}>Archive</Text></Pressable>
                <Pressable style={{width: 30, alignItems: 'center', marginRight: 10}} onPress={() => deleteTask(item.id as string)}><Text style={mainStyles.titleBlock}>X</Text></Pressable>
                </>}
